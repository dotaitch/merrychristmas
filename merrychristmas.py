...
class ChristmasTree(object):
    def __init__(self, n=3, margin_left=10, margin_topbottom=5, time=1):
        self.n = n;
    ...
    def show (self):
        ...
        while(1):
            ...
            for i in range(self.n):
                for j in range(3+i):
                    spaces = " " * (sum(range(self.n+1)) + 1 - sum(range(i+1)) - j + self.margin_left)
                    tree = ''.join(tree_elem[:2*sum(range(i+1))+1+(2*j)])
                    del tree_elem[:(2*(i+j)+1)]
                    print("{}{}".format(spaces, tree))

            n_last_elems = 3 + sum(range(self.n+1)) * 2
